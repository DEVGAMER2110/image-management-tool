...
function createImageElement(familyName, imageData, index) {
    ...
    imageDiv.innerHTML = `
        <img class="image-preview" src="${img.src}" alt="${imageData.currentName}">
        <div class="image-info">
            <input 
                type="text" 
                class="image-name-input" 
                value="${imageData.currentName}"
                onchange="updateImageName('${familyName}', ${index}, this.value)"
            >
            <button 
                class="delete-btn" 
...
function updateImageName(familyName, index, newName) {
    if (imageGroups[familyName] && imageGroups[familyName].images[index]) {
        imageGroups[familyName].images[index].currentName = newName;
    }
}
...
function removeFileExtension(filename) {
    return filename.substring(0, filename.lastIndexOf('.')) || filename;
>>>>>>> a8f24716fd02e44221930dc9e000b5775b06dc5a
}